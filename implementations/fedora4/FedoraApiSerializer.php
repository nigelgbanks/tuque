...
class Fedora4ApiSerializer {
  ...
  /**
   * Serializes the data returned from node GET requests.
   */
  public function getNode($id, $response) {
    $object = $this->getNodeProperties($id, $response);
    // Parents.
    if (isset($object['hasParent'])) {
      $object['parents'] = array();
      foreach ((array) $object['hasParent'] as $id) {
        $object['parents'][$id] = $this->getNodeProperties($id, $response);
      }
    }
    // Children and datastreams.
    if (isset($object['hasChild'])) {
  ...
  /**
   */
  public function getNodeProperties($id, $response) {
    $prefixes = array(
      preg_quote('info:fedora/fedora-system:def/internal#', "/"),
      preg_quote('info:fedora/', "/"),
    );
    ...
    if (isset($response[$id])) {
      foreach ($response[$id] as $property => $values) {
        $property = preg_replace($pattern, '', $property);
        // Some properties have more than one value, some will only ever have
        // one. For now we convert them based on whats there, but later it
        // should be formalized to certian properties.
        if (count($values) == 1) {
          $object[$property] = $values[0]['value'];
        }
        else {
