...
/**
 * @file
 * Test the AbstractObject interface and Tuque's wrapper class that uses it.
 */
...
class AbstractObjectTest extends PHPUnit_Framework_TestCase {
  ...
  protected function setUp() {
    ...
    // @todo Eventually remove API access from the interface.
    $this->api = $this->repository->api;

    // Create an object
    $namespace = random_alpha_numeric_string(10);
    $localname = random_alpha_numeric_string(10);
    $this->testDsid = random_alpha_string(10);
    $this->testPid = "$namespace:$localname";
    $this->api->m->ingest(array('pid' => $this->testPid));
    $this->api->m->addDatastream($this->testPid, $this->testDsid, 'string', '<test> test </test>', NULL);
    $this->object = $this->repository->getObject($this->testPid);
  }
  ...
  public function testCreate() {
    $this->assertTrue(isset($this->object), 'Created a object instance.');
    $this->assertEquals(get_class($this->object), 'Tuque\Object', 'Matched expected type.');
    $this->assertEquals(get_class($this->object->repository), 'Tuque\Repository', 'Matched expected type.');
  }
  ...
  public function testObjectLabel() {
    ...
    $this->object->label = random_alpha_numeric_string(355);
  ...
  public function testDatastreamMutation() {
    ...
    $this->assertTrue($newds instanceof Tuque\NewDatastream, 'Datastream is of expected type.');
    $this->assertTrue($this->object->ingestDatastream($newds) !== FALSE, 'Datastream ingest succeeded.');
    $this->assertTrue($newds instanceof Tuque\Datastream, 'Datastream mutated on ingestion.');
  }
